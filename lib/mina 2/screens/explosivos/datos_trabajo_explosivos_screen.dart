...
import 'package:app_seminco/mina%202/models/PlanTrabajo.dart';
  ...
  Future<void> _getPlanesCompletos() async {
    try {
      final List<PlanMensual> planesMensuales =
          await DatabaseHelper_Mina2().getPlanes();
      final List<PlanProduccion> planesProduccion =
          await DatabaseHelper_Mina2().getPlanesProduccion();
...
void obtenerPlanMensual() async {
  ...
  void _loadData(int id) async {
    var registro = await DatabaseHelper_Mina2().getExploracionById(id);
...
Future<void> _guardarFormulario() async {
  if (_formKey.currentState!.validate()) {
    ...
    var registroExistente =
        await DatabaseHelper_Mina2().getExploracionById(widget.exploracionId);
    if (registroExistente != null) {
      print('🔄 Actualizando registro con ID: ${widget.exploracionId}');
      await DatabaseHelper_Mina2().updateExploracion(widget.exploracionId, datos);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Datos actualizados correctamente')),
      );
    } else {
      print('🆕 Insertando nuevo registro');
      await DatabaseHelper_Mina2().insertExploracionFull(datos);
      ScaffoldMessenger.of(context).showSnackBar(
