...
import 'package:app_seminco/mina%202/screens/explosivos/explosivos_centralizados_screen.dart';
...
String nombreUsuario = "";  // Variable para el nombre completo
  ...
  void _cargarDatos() async {
    List<Map<String, dynamic>> registros = await DatabaseHelper_Mina2().getExploraciones();
    List<Accesorio> accesorios = await DatabaseHelper_Mina2().getAccesorios();
 ...
 int numeroSemana = obtenerNumeroSemana(now);
  String semanaDefault = "Semana $numeroSemana";
    ...
    await DatabaseHelper_Mina2().insertExploracion(fecha, turno, semanaDefault, materialesDespacho, materialesDevolucion);
    _cargarDatos();
  }
...
int obtenerNumeroSemana(DateTime fecha) {
  ...
  Future<void> _eliminarRegistros() async {
    for (int id in _seleccionados) {
      bool resultado = await DatabaseHelper_Mina2().eliminarEstructuraCompletaManual(id);
      if (resultado) {
...
if (firmaUsuario != null && firmaUsuario!.isNotEmpty) {
  ...
  List<Map<String, dynamic>> datos = await DatabaseHelper_Mina2()
