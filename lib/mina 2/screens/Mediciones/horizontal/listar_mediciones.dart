...
class _ListaPantallaState extends State<ListaPantalla> {
  ...
  void _cargarMediciones() async {
    try {
      ...
      final datos = await DatabaseHelper_Mina2().obtenerTodasMedicionesHorizontal();
...
Future<void> _eliminarSeleccionadas() async {
  ...
  if (confirm == true) {
    try {
      ...
      await DatabaseHelper_Mina2().actualizarMedicionExplosivoACero(idsExplosivo);

      // ✅ Eliminar las mediciones seleccionadas
      await DatabaseHelper_Mina2().eliminarMultiplesMedicionesLargo(_selectedIds.toList());
