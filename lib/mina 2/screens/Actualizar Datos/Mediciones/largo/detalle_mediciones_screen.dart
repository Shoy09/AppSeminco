...
import 'package:app_seminco/mina%202/models/Envio%20Api/medicion_largo.dart';
import 'package:app_seminco/mina%202/services/Enviar%20nube/ExploracionService_service.dart';
import 'package:app_seminco/mina%202/services/Enviar%20nube/api_service_mediciones_largo.dart';
...
class ListaMedicionesLargoScreen extends StatefulWidget {
  final String tipoPerforacion;

  ListaMedicionesLargoScreen({required this.tipoPerforacion});

  @override
  _DetalleSeccionScreenState createState() => _DetalleSeccionScreenState();
}

class _DetalleSeccionScreenState extends State<ListaMedicionesLargoScreen> {
  ...
  Future<void> _fetchMedicionesData() async {
    ...
    try {
      ...
      List<Map<String, dynamic>> data = await dbHelper.obtenerTodasMedicionesLargo();
  ...
  Future<void> _exportSelectedItems() async {
    ...
    for (var id in selectedItems) {
      Map<String, dynamic>? medicion = await dbHelper.obtenerMedicionLargoPorId(id);
...
Future<void> _enviarDatosALaNube(List<Map<String, dynamic>> jsonData) async {
  final medicionService = ApiServiceMedicionesLargo();
  ...
  try {
    // Paso 1: Enviar todas las mediciones largas
    for (var medicionMap in jsonData) {
      try {
        final medicion = MedicionLargo.fromJson(medicionMap);
        bool success = await medicionService.postMedicionLargo(medicion.toApiJson());
        ...
        if (success) {
          ...
          allSuccess = false;
          errores.add("Error al enviar medición larga ID: ${medicionMap['id']}");
        }
      } catch (e) {
        allSuccess = false;
        errores.add("Error procesando medición larga ID: ${medicionMap['id']} - ${e.toString()}");
    ...
    if (idsNubeParaMarcar.isNotEmpty) {
      ...
      if (!marcadoExitoso) {
        ...
        print('✅ ${idsNubeParaMarcar.length} registros marcados como usados en mediciones');
      }
    }
  ...
  Future<void> _eliminarRegistrosSeleccionados() async {
    ...
    try {
      DatabaseHelper_Mina2 dbHelper = DatabaseHelper_Mina2();
      int deletedCount = await dbHelper.eliminarMultiplesMedicionesLargo(selectedItems.toList());
  ...
  Future<int> _actualizarEnvio(int medicionId) async {
    DatabaseHelper_Mina2 dbHelper = DatabaseHelper_Mina2();
    return await dbHelper.actualizarEnvioMedicionesLargo([medicionId]);
  }
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Text(
              success
                  ? 'Las mediciones largas se enviaron correctamente y los registros se marcaron como usados.'
                              ...
                              child: ListTile(
                                ...
                                subtitle: Column(
                                  ...
                                  children: [
                                    ...
                                    Text("Toneladas: ${medicion['toneladas']}"), 
