...
// Función para formatear el número
  ...
  void _loadDetallesDespachoExplo(int despachoId) async {
    List<Map<String, dynamic>> detalles =
        await DatabaseHelper_Mina1().getDetalleDespachoByDespachoId(despachoId);
  ...
  Future<bool> _actualizarDespacho() async {
    ...
    int result =
        await DatabaseHelper_Mina1().updateDespacho(_despachoId!, updatedData);
    if (result > 0) {
      return true; // Éxito
    } else {
  ...
  Future<bool> _guardarFormulario() async {
    ...
    if (detalles.isNotEmpty) {
      await DatabaseHelper_Mina1().insertDetallesDespacho(_despachoId!, detalles);
      return true; // Éxito
    } else {
  ...
  Future<void> _actualizarTiempos() async {
    ...
    int filasActualizadas = await DatabaseHelper_Mina1().actualizarTiemposDespacho(
      _despachoId!,
      ms,
      lp,
    );
