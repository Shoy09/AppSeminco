...
class _FormularioDevolucionesScreenState
  ...
  void _cargarDatos() async {
    List<Map<String, String>> accesorios =
        await DatabaseHelper_Mina1().getAccesoriosunidad();
    List<Map<String, String>> explosivos =
        await DatabaseHelper_Mina1().getExplosivosunidad();
  ...
  void fetchExplosivosuni() async {
    List<ExplosivosUni> explosivos = await DatabaseHelper_Mina1().getExplosivosUni();
  ...
  void _loadDetallesDevoluciones() async {
    List<Map<String, dynamic>> detalles = await DatabaseHelper_Mina1()
  ...
  Future<void> generatePdf() async {
    ...
    List<Map<String, dynamic>> datos =
        await DatabaseHelper_Mina1().obtenerEstructuraCompleta(widget.exploracionId);
...
//-------------------------------------------------------------------------------------------------------
  ...
  Future<bool> _actualizarDevoluciones() async {
    ...
    int result = await DatabaseHelper_Mina1()
  ...
  Future<void> _actualizarTiempos() async {
    ...
    int filasActualizadas =
        await DatabaseHelper_Mina1().actualizarTiemposDevoluciones(
      _DevolucionesId!,
      ms,
      lp,
    );
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                // 🔹 Llamar directamente a la función en DatabaseHelper_Mina1
                await DatabaseHelper_Mina1().cerrarRegistro(widget.exploracionId);
  ...
  Future<bool> _guardarFormulario() async {
    ...
    List<Map<String, dynamic>> detallesDespacho =
        await DatabaseHelper_Mina1().getDetalleDespachoByDespachoId(_despachoId!);
    ...
    for (int i = 1; i <= 20; i++) {
      ...
      if (msCant1.isNotEmpty || lpCant1.isNotEmpty) {
        ...
        if (msDevolucion > msDespacho) {
          throw Exception(
              'Error en : La cantidad de devolución en el número $i es mayor que la del despacho.');
        }
        if (lpDevolucion > lpDespacho) {
          throw Exception(
              'Error en Medio Segundo: La cantidad de devolución en el número $i es mayor que la del despacho.');
        }
