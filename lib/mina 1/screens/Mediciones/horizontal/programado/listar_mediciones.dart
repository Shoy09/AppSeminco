...
class _ListaPantallaState extends State<ListaPantalla> {
  ...
  void _cargarMediciones() async {
    try {
      ...
      final datos = await DatabaseHelper_Mina1().obtenerTodasMedicionesHorizontalProgramado();
...
Future<void> _eliminarSeleccionadas() async {
  ...
  if (confirm == true) {
    try {
      ...
      await DatabaseHelper_Mina1().actualizarMedicionExplosivoACeroProgramado(idsExplosivo);

      // ✅ Eliminar las mediciones seleccionadas
      await DatabaseHelper_Mina1().eliminarMultiplesMedicionesHorizontalProgramado(_selectedIds.toList());
